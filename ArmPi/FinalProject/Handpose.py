...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 下面三行可以根据自己的电脑进行调节
        src = cv2.resize(frame, (800, 700), interpolation=cv2.INTER_CUBIC)  # 窗口大小
        cv2.rectangle(src, (0, 0), (800, 700), (0, 255, 0))  # 框出截取位置
        roi = src[0:800, 0:700]  # 获取手势框图
        resb = remove_background(roi)
        res = skin_color(resb)  # 进行肤色检测
        cv2.imshow("0", resb)
        ...
        contour, array = get_contour(Laplacian)  # 轮廓处理
        cv2.imshow("2", contour)

        largecont = max(array, key=lambda contour: cv2.contourArea(contour))
        frame_res, finger_num = get_convex(frame, largecont)
